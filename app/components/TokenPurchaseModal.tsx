...
const TokenPurchaseModal: React.FC<TokenPurchaseModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-80 flex items-center justify-center z-[2000] p-4 md:p-6 backdrop-blur-sm overflow-y-auto">
      <div className="bg-gradient-to-br from-blue-900 to-indigo-900 text-white rounded-2xl md:rounded-3xl shadow-2xl w-full max-w-4xl p-4 sm:p-6 md:p-8 relative border border-blue-400/20 my-4 md:my-0">
        {/* Close button - positioned absolutely */}
        <button
          onClick={onClose}
          className="absolute top-3 right-3 md:top-6 md:right-6 text-gray-300 hover:text-white bg-blue-800/50 p-1.5 md:p-2 rounded-lg hover:bg-blue-700/70 transition-all z-[3000]"
        ...
        <div className="flex items-center gap-2 md:gap-3 mb-4 md:mb-6">
          <div className="bg-gradient-to-r from-blue-500 to-indigo-600 p-2 md:p-3 rounded-lg md:rounded-xl shadow-lg">
            <Wallet size={20} className="md:hidden text-white" />
            <Wallet size={28} className="hidden md:block text-white" />
          </div>
          <h2 className="text-xl md:text-3xl font-bold bg-gradient-to-r from-blue-200 to-indigo-100 bg-clip-text text-transparent">
            Buy QSE Tokens
          </h2>
        </div>
        ...
        <div className="flex flex-col">
          {/* Wallet info bar */}
          {isConnected && (
            <div className="mb-4 md:mb-6 flex justify-between items-center p-3 md:p-4 bg-blue-800/40 rounded-lg md:rounded-xl text-xs md:text-sm border border-blue-500/30 backdrop-blur-md">
              ...
              <div className="bg-blue-950/70 px-2 md:px-3 py-1 md:py-1.5 rounded-lg text-xs md:text-sm text-blue-300 font-mono truncate max-w-[150px] md:max-w-[200px]">
                {account
          ...
          {purchaseCompleted ? (
            <div className="text-center py-6 md:py-10 animate-fadeIn">
              <div className="inline-flex items-center justify-center w-16 h-16 md:w-24 md:h-24 mb-4 md:mb-6 rounded-full bg-gradient-to-br from-green-400 to-green-600">
                <CheckCircle size={36} className="md:hidden text-white" />
                <CheckCircle size={48} className="hidden md:block text-white" />
              </div>
              <h3 className="text-xl md:text-2xl font-bold mb-4 text-white">
                Purchase Successful!
              </h3>
              <div className="bg-blue-800/40 border border-blue-500/30 rounded-xl p-4 md:p-6 mb-4 md:mb-8 max-w-md mx-auto">
                <div className="flex justify-between items-center mb-2 pb-2 border-b border-blue-700/50">
              ...
              {txHash && (
                <div className="mb-4 md:mb-6 p-3 md:p-4 bg-blue-900/60 rounded-xl text-xs break-all max-w-md mx-auto border border-blue-700/50">
              ...
              <button
                onClick={onClose}
                className="px-6 md:px-8 py-2.5 md:py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-medium transition-all shadow-lg hover:shadow-blue-500/30"
              >
                Close
              </button>
            </div>
            ...
            <div>
              ...
              <form onSubmit={handleSubmit} className="space-y-4 md:space-y-6">
                ...
                <div>
                  <label className="block text-xs md:text-sm font-medium mb-1.5 md:mb-2 text-blue-200">
                    Payment Method
                  </label>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <input
                        ...
                        className="w-full bg-blue-900/60 border border-blue-700/50 rounded-lg md:rounded-xl py-2.5 md:py-3.5 px-3 md:px-4 text-white text-sm md:text-base focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 placeholder-gray-400"
                        placeholder="0.00"
                        required
                        disabled={!isConnected}
                      />
                      <div className="absolute right-3 md:right-4 top-1/2 transform -translate-y-1/2 text-blue-300 font-medium text-xs md:text-base">
                        {selectedPaymentMethod}
                      </div>
                    </div>
                ...
                {/* Transaction Details - Show when needed */}
                {receiveAmount && (
                  <div className="bg-blue-800/40 border border-blue-500/30 rounded-lg md:rounded-xl p-3 md:p-6 backdrop-blur-sm text-sm">
                    <h4 className="font-medium mb-2 md:mb-3 text-blue-100 text-xs md:text-sm">
                      Transaction Details:
                    </h4>
                    <div className="space-y-1.5 md:space-y-2">
                      ...
                      <div className="h-px bg-blue-700/50 my-1.5 md:my-2"></div>
                ...
                {errorMessage && (
                  <div
                    className={`p-3 md:p-4 rounded-lg md:rounded-xl text-xs md:text-sm flex items-start gap-2 md:gap-3 ${
                  ...
                  >
                    {errorMessage.includes("success") ? (
                      <CheckCircle
                        size={16}
                        className="md:hidden text-green-400 mt-0.5"
                      />
                    ) : (
                      <AlertCircle
                        size={16}
                        className="md:hidden text-red-400 mt-0.5"
                      />
                    )}
                    {errorMessage.includes("success") ? (
                      <CheckCircle
                        size={18}
                        className="hidden md:block text-green-400 mt-0.5"
                      />
                    ) : (
                      <AlertCircle
                        size={18}
                        className="hidden md:block text-red-400 mt-0.5"
                      />
                    )}
                ...
                <div className="!mt-6 md:!mt-8">
                  <button
                    ...
                    className={`w-full py-3 md:py-4 rounded-xl font-semibold text-white bg-gradient-to-r from-blue-500 to-indigo-600 hover:from-blue-600 hover:to-indigo-700 transition-all shadow-lg hover:shadow-blue-500/30 relative overflow-hidden group ${
                    ...
                    <span className="relative z-10 flex items-center justify-center gap-2">
                      {isSubmitting ? (
                        ...
                        <>
                          ...
                          <ArrowRight
                            size={18}
                            className="hidden md:block transition-transform group-hover:translate-x-1"
                          />
                        </>
                      ) : (
                        <>
              ...
              {/* Quick Buy Section */}
              <div className="mt-6 pt-6 border-t border-blue-700/40">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 mb-4">
                  {quickBuyOptions.map((amount) => (
                    <button
                      ...
                      disabled={!isConnected}
                      className={`flex flex-col justify-center items-center p-3 rounded-lg border transition-all ${
                ...
                <button
                  ...
                  className={`w-full py-3 rounded-lg md:rounded-xl font-medium text-white bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700 flex items-center justify-center gap-2 transition-all ${
